...
//            "description=#{question.description},image=#{question.image},publishTime=#{question.publish_time} where id=#{question.id}")
...
//    int delete(@Param("id") Integer id);
    ...
    @Select("select q.*,u.image AS userImage from question q left join user u on u.username = q.username order by publish_time DESC")
    List<QuestionDto> getAll();

    @Select("select q.*,u.image AS userImage from question q left join user u on u.username = q.username where type=#{type} order by publish_time DESC")
