 ...
 * Describe: 百科类Fg P层
 */
public class BaikeFeedFgPresenter extends BasePresenter<BaikeFeedFgView> {

    public BaikeFeedFgPresenter(BaseActivity context) {
        super(context);
    }

    @SuppressLint("CheckResult")
    public void queryKindBaike( int pageNum) {
        ApiRetrofit.getInstance().queryKindWeiBaike(pageNum)
    ...
    public void deleteBaikeByID(int baikeID, int itemPostion){
        ...
        ApiRetrofit.getInstance().deleteFeedBaikeByID(baikeID)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
