 ...
 * Describe: 百科类Fg P层
 */
public class BaikeDiseaseFgPresenter extends BasePresenter<BaikeDiseaseFgView> {

    public BaikeDiseaseFgPresenter(BaseActivity context) {
        super(context);
    }

    @SuppressLint("CheckResult")
    public void queryDiseaseBaike(int pageNum) {
        ApiRetrofit.getInstance().queryKindBingBaike(pageNum)
    ...
    public void deleteBaikeByID(int baikeID, int itemPostion){
        ...
        ApiRetrofit.getInstance().deleteDiseaseBaikeByID(baikeID)
                .subscribeOn(Schedulers.io())
                .observeOn(AndroidSchedulers.mainThread())
